...
import numpy as np # csv file save & load
import string       # texts 전처리
from konlpy.tag import Okt # 한글 형태소 추출기
...
# (2-2) 결과값 csv로 저장 및 불러오기
df = pd.DataFrame(rpl_result)
df.to_csv('rpl_result.csv', index=False)
rpl_result = pd.read_csv('rpl_result.csv', encoding=('utf-8'))
rpl_result = np.array(rpl_result).flatten().tolist()
